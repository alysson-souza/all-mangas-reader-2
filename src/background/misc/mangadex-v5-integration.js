...
export class Mangadex extends EventEmitter {
        ...
        for (const r of res) {
            ...
            langs.forEach(l => {
                const lastRead = l.chapters.reduce((prev, curr) => {
                    return Math.abs(parseFloat(curr.chapNum) - parseFloat(lastChaptersRead)) <
                        Math.abs(parseFloat(prev.chapNum) - parseFloat(lastChaptersRead))
                        ? curr
                        : prev
                })
    ...
    async getAllchapters(id, inStore) {
        ...
        // loop
        for (let page = 0; page <= 100000; page++) {
            const resp = await this.MD(
                `/manga/${id}/feed?limit=100&offset=${
                    page * 100
                }&order[chapter]=desc&contentRating[]=safe&contentRating[]=suggestive&contentRating[]=erotica&contentRating[]=pornographic`,
                "GET"
            )
            const current = parseInt(resp.limit) + parseInt(resp.offset)
            const total = parseInt(resp.total)
            ...
            for (const data of resp.data) {
                ...
                const titleParts = []
                if (data.attributes.chapter && data.attributes.chapter.length > 0)
                    titleParts.push(data.attributes.chapter)
        ...
        Object.keys(res).forEach(k => {
            const chapters = Object.values(
                res[k].reduce((a, { id, chapNum, title, url, date }) => {
                    if (a[id]) {
