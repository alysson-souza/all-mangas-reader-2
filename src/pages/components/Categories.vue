<template>
    <v-menu :close-on-content-click="false" v-model="menu">
        <template #activator="{ on: onMenu }">
            <v-tooltip top>
                <template #activator="{ on: onTooltip }">
                    <v-btn v-on="{ ...onMenu, ...onTooltip }" text small :ripple="false" class="no-bg-hover">
        ...
        <v-card>
            <v-row no-gutters>
                <v-btn color="gray" @click="menu = false" class="ml-auto no-bg-hover" text>
            ...
            <v-row no-gutters class="px-4">
                ...
                <div
                    v-for="(cat, key) in sortedCategories"
                    :class="'cat-chip ' + (staticCats ? 'include' : cat.state)"
                    :key="key"
                    @click="switchState(cat)">
                    <!-- Category name and tooltip -->
                    <v-tooltip
                        v-if="!staticCats && cat.type !== 'language'"
                        top
                        content-class="icon-ttip"
                        class="cat-name">
                        <template v-slot:activator="{ on }">
                            <span class="cat-name" v-on="on">{{
                                cat.type === "native" ? i18n(cat.name) : cat.name
                            }}</span>
                        </template>
                        <span
                            v-if="cat.state === 'include'"
                            v-html="i18n('list_cat_include', cat.type === 'native' ? i18n(cat.name) : cat.name)"></span>
                        <span
                            v-if="cat.state === 'exclude'"
                            v-html="i18n('list_cat_exclude', cat.type === 'native' ? i18n(cat.name) : cat.name)"></span>
                        <span
                            v-if="!cat.state"
                            v-html="i18n('list_cat_no', cat.type === 'native' ? i18n(cat.name) : cat.name)"></span>
                    </v-tooltip>
                    ...
                    <v-tooltip top content-class="icon-ttip">
                        <template v-slot:activator="{ on }">
                            <v-icon
                                v-if="cat.type !== 'native' && cat.type !== 'language'"
                                class="cat-act"
                                @click.stop="displayEditCategoryModal(cat)"
                                v-on="on"
                                >mdi-pencil</v-icon
                            >
                        </template>
            ...
            <v-dialog v-model="deleteCatDialog" max-width="290">
                <v-card>
                    ...
                    <v-card-actions>
                        <v-spacer></v-spacer>
                        <v-btn color="red darken-1" text @click.native="reallyDeleteCat()">{{
                            i18n("button_yes")
                        }}</v-btn>
                        <v-btn color="grey darken-1" text @click.native="deleteCatDialog = false">{{
                            i18n("button_no")
                        }}</v-btn>
                    </v-card-actions>
                </v-card>
            ...
            <v-dialog v-model="editCatDialog" max-width="290">
                <v-card>
                    ...
                    <v-card-actions>
                        ...
                        <v-btn
                            color="red darken-1"
                            :disabled="!isValidEditName"
                            text
                            @click.native="submitEditCategory()">
                            {{ i18n("button_yes") }}
                        </v-btn>
                        <v-btn color="grey darken-1" text @click.native="editCatDialog = false">{{
                            i18n("button_no")
                        }}</v-btn>
                    </v-card-actions>
                </v-card>
...
export default {
    ...
    computed: {
        allincluded: function () {
            return (
                this.categories.reduce((nb, cat) => (cat.state === "include" ? nb + 1 : nb), 0) ===
...
.cat-chip.include:hover,
.cat-chip.exclude:hover {
    background: #a0a0a0;
}
.theme--dark .cat-chip {
    background: #424242;
...
.theme--dark .cat-chip.include,
.theme--dark .cat-chip.exclude {
    background: #707070;
}
.theme--dark .cat-chip.include:hover,
.theme--dark .cat-chip.exclude:hover {
    background: #424242;
}
