<template>
    <div>
        ...
        <div v-if="enabled && (!validCredentials || tokenExpired)">
            <v-text-field v-model="login" :label="i18n('options_mangadex_login')"></v-text-field>
            <v-text-field type="password" v-model="password" :label="i18n('options_mangadex_password')"></v-text-field>
            <!-- Confirm credentials button -->
            <v-btn :loading="credentialLoading" v-if="login && password" @click="verifyCredentials">
                {{ i18n("options_mangadex_verify_credentials") }}</v-btn
            >
        </div>
        ...
        <v-btn
            ...
            v-if="validCredentials && !tokenExpired"
            @click="resetCredentials">
            {{ i18n("options_mangadex_reset_credentials") }}</v-btn
        >
        ...
        <div class="mt-4" v-if="enabled && validCredentials && !tokenExpired">
            <v-checkbox v-model="updateReadMarker" :label="i18n('options_mangadex_integration_markwhendownload')" />
            ...
            <v-row v-if="follows.length">
                ...
                <v-col v-if="!addLangLoading || addLangLoading == ''" cols="12" lg="9">
                    <v-card class="pb-6">
...
export default {
    ...
    computed: {
        ...
        datasaver: {
            get() {
                return this.allOptions.mangadexDataSaver == 1
            },
            set(val) {
        ...
        updateReadMarker: {
            get() {
                return this.options.markAsRead
            },
            set(val) {
        ...
        updateExportToListMarker: {
            get() {
                return this.options.exportToList
            },
            set(val) {
        ...
        updateExportToFollowsMarker: {
            get() {
                return this.options.exportToFollows
            },
            set(val) {
        ...
        loadings() {
            return this.$store.getters.md_loadings
        },
        texts() {
            return this.$store.getters.md_texts
        },
        misc() {
            return this.$store.getters.md_misc
        },
        credentialLoading() {
            return this.loadings.credential
        },
        exportLoading() {
            return this.loadings.export
        },
        exportLoadingText() {
            return this.texts.export
        },
        exportProgress() {
            return this.texts.exportProgress
        },
        exportTotal() {
            return this.texts.exportTotal
        },
        exportDone() {
            return this.misc.exportDone
        },
        exportFollowsLoading() {
            return this.loadings.exportFollows
        },
        exportFollowsLoadingText() {
            return this.texts.exportFollows
        },
        exportFollowsProgress() {
            return this.texts.exportFollowsProgress
        },
        exportFollowsTotal() {
            return this.texts.exportFollowsTotal
        },
        exportFollowsDone() {
            return this.misc.exportFollowsDone
        },
        importLoading() {
            return this.loadings.import
        },
        importButtonText() {
            return this.follows.length ? "options_mangadex_integration_reload" : "options_mangadex_integration_load"
        },
        importLoadingText() {
            return this.texts.import
        },
        importProgress() {
            return this.texts.importProgress
        },
        importTotal() {
            return this.texts.importTotal
        },
        addLangLoading() {
            return this.loadings.importLang
        },
        follows() {
            return this.$store.getters.md_imports
        },
        followsLangs() {
            return Array.from(
                new Set(
                ...
                .map(l => {
                    return {
                        code: l,
                        count: this.follows.filter(follow =>
                            follow.langs.find(lang => lang.code === l && lang.chapters.length)
                        ).length
                    }
                })
    ...
    methods: {
        ...
        async verifyCredentials() {
            browser.runtime.sendMessage({
                action: "mangadexVerifyCredentials",
                username: this.login,
        ...
        addManga(item, selectedSubitem) {
            const payload = {
                ...
                lastChapterReadURL: selectedSubitem.lastChapterReadURL
                    ? selectedSubitem.lastChapterReadURL
                    : selectedSubitem.chapters[0][1],
                lastChapterReadName: selectedSubitem.lastChapterReadName
                    ? selectedSubitem.lastChapterReadName
                    : selectedSubitem.chapters[0][0],
