...
if (typeof registerMangaObject === 'function') {
    registerMangaObject({
        ...
        getMangaList: async function (search) {
            ...
            return Object.assign({}, ...(await Promise.all(categories.map(async ele => {
                let data = await amr.loadJson(this.apiUrl() + 'category/' + ele.id)
                return data.map(elem => {
                    return [
                        elem.title,
                        new URL('manga/' + elem.id, amr.getOption('tachideskUrl')).href
                    ]
                })
            }))))
        },
        ...
        getListChaps: async function (urlManga) {
            let id = urlManga.split("/").filter(ele => ele!='').slice(-1)[0]
            let data
            try {
            ...
            return data.map(ele => {
                return [
                    ele.name,
                    new URL('manga/' + id + '/chapter/' + ele.index, amr.getOption('tachideskUrl')).href
                ]
            })
        },

        getInformationsFromCurrentPage: async function (doc, curUrl) {
            let id = curUrl.split("/").filter(ele => ele!='').slice(-3)[0]
            let chapid = curUrl.split("/").filter(ele => ele!='').slice(-1)[0]
        ...
        getListImages: async function (doc, curUrl) {
            ...
            return Array(chapdat.pageCount).fill(0).map((_,pageNumber) =>{
        ...
        isCurrentPageAChapterPage: function (doc, curUrl) {
            return curUrl.split("/").filter(ele => ele!='').slice(-2)[0] == "chapter"
        }
    });
}
