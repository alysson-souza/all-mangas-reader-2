    ...
    registerMangaObject({
        ...
        getInformationsFromCurrentPage: async function (doc, curUrl) {
            var name = $("select[name='manga'] option:selected", doc).text(),
                currentMangaURL =
                    "http://read.egscans.com/" + $("select[name='manga'] option:selected", doc).attr("value"),
                currentChapterURL =
                    currentMangaURL + "/" + $("select[name='chapter'] option:selected", doc).attr("value")
            return {
                name: name,
        ...
        getListImages: async function (doc, curUrl) {
            var res = [],
                ...
                i
            txt = $(
                ".mid > table:nth-child(1) > tbody:nth-child(1) > tr:nth-child(3) > td:nth-child(1) > script:nth-child(2)",
                doc
            ).html()
            txt = JSON.stringify(txt)
            img = txt.match(/img_url\.push\(\'.+?\' \)/g)
            for (i = 0; img.length > i; i += 1) {
                res.push("http://read.egscans.com/" + img[i].match(/'(.*?)'/gi)[0].replace(/'/g, ""))
            }
            return res
        },
