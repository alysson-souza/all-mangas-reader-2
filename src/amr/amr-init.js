...
let updateApp = async function (ancVersion, curVersion) {
    let afterCalls = []
    if (!versionAfter(ancVersion, "2.0.2.140")) {
    ...
    if (!versionAfter(ancVersion, "2.0.2.150")) {
        ...
        afterCalls.push(async () => {
            await window["AMR_STORE"].dispatch("updateLanguageCategories")
        })
    }
    if (!versionAfter(ancVersion, "2.0.4")) {
    ...
    if (versionAfter(ancVersion, "2.3.11")) {
        // Just checking if it exists for those who skip versions of AMR
        afterCalls.push(async () => {
            if (
                !window["AMR_STORE"].state.options.categoriesStates.find(
                    cat => cat.name === "category_disabled_mirrors"
                )
            ) {
...
let browserdetect = function () {
    ...
    return isOpera ? "Opera" : isFirefox ? "Firefox" : "Chrome" // no good way to detect chrome browser in an extension
}
