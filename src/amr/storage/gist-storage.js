...
export default class GistStorage extends Storage {
    ...
    async getAll() {
        ...
        const request = await this.ax("get", `gists/${this.gistSyncGitID}?cache=${Date.now()}`).catch(
    ...
    async init() {
        await this.wait()
        const request = await this.ax("patch", `gists/${this.gistSyncGitID}`, this.getFileStruct("[]")).catch(
            this.handleSyncError
        )
    ...
    async saveAll(content) {
        await this.wait()
        return this.ax("patch", `gists/${this.gistSyncGitID}`, this.getFileStruct(JSON.stringify(content))).catch(
            this.handleSyncError
        )
    }
